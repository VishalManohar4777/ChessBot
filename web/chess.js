 ...
 *
 * This is a direct port of the C++ engine's mailbox move generator so
 * the browser UI enforces exactly the same rules. The C++ engine (via
 * the Python bridge) is only consulted for the *computer's* move.
 *
 * Piece encoding (same as C++): white = +1..+6, black = -1..-6, 0 empty.
 * Works both in the browser (globals) and in Node (module.exports).
 * ------------------------------------------------------------------ */
(function (root) {
  "use strict";
  ...
  function squareName(sq) {
    return String.fromCharCode(97 + (sq % 8)) + String.fromCharCode(49 + (sq >> 3));
  }
  ...
  function startPosition() {
    ...
    s.ep = -1; s.halfmove = 0; s.fullmove = 1;
    return s;
  }
  ...
  function isSquareAttacked(s, square, byColor) {
    ...
    for (let i = 0; i < 8; i++) {
      const n = MAILBOX[base + KNIGHT_OFF[i]];
      if (n !== -1 && b[n] === knight) return true;
    }

    const king = byColor === WHITE ? KING : -KING;
    for (let i = 0; i < 8; i++) {
      const n = MAILBOX[base + KING_OFF[i]];
      if (n !== -1 && b[n] === king) return true;
    }
  ...
  function inCheck(s, color) {
    return isSquareAttacked(s, kingSquare(s, color), color ^ 1);
  }
      ...
      if (type === PAWN) {
        ...
        if (one >= 0 && one < 64 && b[one] === 0) {
          ...
          if (!capturesOnly && rank === startRank) {
            ...
            if (b[two] === 0) moves.push(mk(from, two, p, 0, FLAG_DOUBLE, 0));
          }
        }

        const caps = [dir - 1, dir + 1];
        const guard = [file > 0, file < 7];
        for (let i = 0; i < 2; i++) {
          if (!guard[i]) continue;
        ...
        for (let i = 0; i < 8; i++) {
          ...
          if (n === -1) continue;
          const target = b[n];
          if (target === 0) { if (!capturesOnly) moves.push(mk(from, n, p, 0, FLAG_NORMAL, 0)); }
          else if (pieceColor(target) === them) moves.push(mk(from, n, p, target, FLAG_NORMAL, 0));
        }
      } else if (type === KING) {
        for (let i = 0; i < 8; i++) {
          const n = MAILBOX[base + KING_OFF[i]];
  ...
  function makeMove(s, m) {
    ...
    if (m.flag === FLAG_PROMO) b[to] = color === WHITE ? m.promotion : -m.promotion;
    else b[to] = m.piece;

    if (m.flag === FLAG_CASTLE_K) {
      if (color === WHITE) { b[5] = b[7]; b[7] = 0; } else { b[61] = b[63]; b[63] = 0; }
    } else if (m.flag === FLAG_CASTLE_Q) {
      if (color === WHITE) { b[3] = b[0]; b[0] = 0; } else { b[59] = b[56]; b[56] = 0; }
    }

    if (m.flag === FLAG_DOUBLE) ns.ep = color === WHITE ? from + 8 : from - 8;

    if (pieceType(m.piece) === KING) {
      if (color === WHITE) ns.castling &= ~(C_WK | C_WQ);
      else ns.castling &= ~(C_BK | C_BQ);
    }
    const touch = (sq) => {
      if (sq === 0) ns.castling &= ~C_WQ;
      else if (sq === 7) ns.castling &= ~C_WK;
      else if (sq === 56) ns.castling &= ~C_BQ;
      else if (sq === 63) ns.castling &= ~C_BK;
    };
    touch(from); touch(to);

    if (pieceType(m.piece) === PAWN || m.captured !== 0) ns.halfmove = 0;
    else ns.halfmove++;
  ...
  function pieceToChar(p) {
    let c = ".";
    switch (pieceType(p)) {
      case PAWN: c = "p"; break; case KNIGHT: c = "n"; break;
      case BISHOP: c = "b"; break; case ROOK: c = "r"; break;
  ...
  function toFEN(s) {
    ...
    for (let rank = 7; rank >= 0; rank--) {
      ...
      for (let file = 0; file < 8; file++) {
        const p = s.board[rank * 8 + file];
        if (p === 0) empty++;
        else { if (empty) { fen += empty; empty = 0; } fen += pieceToChar(p); }
      }
      if (empty) fen += empty;
      if (rank) fen += "/";
    }
    ...
    let c = "";
    if (s.castling & C_WK) c += "K";
    if (s.castling & C_WQ) c += "Q";
    if (s.castling & C_BK) c += "k";
    if (s.castling & C_BQ) c += "q";
    fen += c || "-";
    fen += " " + (s.ep === -1 ? "-" : squareName(s.ep));
  ...
  function moveToUci(m) {
    ...
    if (m.flag === FLAG_PROMO) {
      s += { [QUEEN]: "q", [ROOK]: "r", [BISHOP]: "b", [KNIGHT]: "n" }[m.promotion] || "q";
    }
    return s;
  }

  // Find a legal move object matching a UCI string ("e2e4", "e7e8q").
  function moveFromUci(s, uci) {
    ...
    for (const m of legalMoves(s)) {
      if (m.from === from && m.to === to) {
        if (m.flag === FLAG_PROMO) {
          const want = promoChar ? promoMap[promoChar] : QUEEN;
  ...
  // perft for self-testing the rules.
  ...
  const API = {
    ...
    legalMoves, legalMovesFrom, makeMove, status, toFEN, moveToUci, moveFromUci, perft
  };
